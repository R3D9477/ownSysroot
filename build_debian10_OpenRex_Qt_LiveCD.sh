...
export IMG_DEV_DISTRO="${IMG_NAME}"
export IMG_SIZE_MB=$((IMG_SIZE_MB+500))
export IMG_NAME="${IMG_DEV_DISTRO%.*}-livecd.img"

pushd "${COREDIR}"
    if ! ( bash "03-create_bootable_img.sh" )               ; then showElapsedTime ; exit 11 ; fi
    if ! ( bash "04-mount_img.sh" )                         ; then showElapsedTime ; exit 12 ; fi
    if ! ( bash "05-sysroot_debian10_create_base.sh" )      ; then showElapsedTime ; exit 13 ; fi
    if ! ( bash "07-install_kernel.sh" )                    ; then showElapsedTime ; exit 14 ; fi
    if ! ( bash "08-install_uboot.sh" )                     ; then showElapsedTime ; exit 15 ; fi
    if ! ( bash "09-sysroot_configure.sh" )                 ; then showElapsedTime ; exit 16 ; fi
popd

pushd "${USERDIR}"
    if ! ( bash "${USERDIR}/install_MMC_installer.sh" )     ; then showElapsedTime ; exit 17 ; fi
popd
...
if ! ( bash "core/10-flash_image.sh" )                      ; then showElapsedTime ; exit 18 ; fi
